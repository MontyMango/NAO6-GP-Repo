...
def set_personality(user_choice):
    ...
    if user_choice == 0:
        try:
            prompt_path = os.path.join(base_dir, "../system_prompts/wills_POLITE_system_prompt.txt")  # Construct the absolute path
        ...
        try:
            prompt_path = os.path.join(base_dir, "../system_prompts/wills_SNARKY_system_prompt.txt")  # Construct the absolute path
            with open(prompt_path, "r") as file:
                system_prompt = file.read()
        except FileNotFoundError:
            raise FileNotFoundError(f"File not found: {prompt_path}")
